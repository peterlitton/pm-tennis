...
**Last updated:** H-018 session close (2026-04-20)
...
| **I-016** | **`event_date` field empty in meta.json across Phase 2 discovery output. Surfaced at H-015 during Diagnostic B in the `pm-tennis-api` Render Shell while investigating an empty `slug_selector.list_candidates()` result. 10 most-recent meta.json files (event_ids 9440–9449) all show `event_date=""` (empty string) despite event titles encoding the date as text (e.g., "Elmer Moeller vs. Hugo Gaston 2026-04-20"). Root cause likely at `src/capture/discovery.py` line 328: `event_date=str(event.get("eventDate") or "").strip()` — if Polymarket gateway responses do not include a top-level `eventDate` key (or use a different key name), the extracted value is empty. `slug_selector._passes_date_filter` (lines 142–156) correctly rejects empty `event_date` as defensive behavior; the issue is upstream extraction, not selector logic. Operator confirmed at H-015 that the 10 surfaced events are future games (event titles dated 2026-04-20+), so the slug-selector rejection is functionally correct today (no eligible matches in the next ~24h regardless), but the bug means slug_selector will continue rejecting eligible matches as they reach the pre-match window if `event_date` remains empty — could block H-016 probe retry and any subsequent stress-test work that relies on the candidate-listing path.** | **6** | **H-016 first action (before probe retry): read `src/capture/discovery.py` `_event_to_meta` extraction (around line 328) and a sample raw gateway event payload to identify the correct key name for the date. Likely fix candidates include `startDate`, `startTime`, derived from a different field, or extracted from a nested participant/market structure. Phase 2 code is preserved per D-016 — any actual fix is a Phase-2-touching change requiring operator authorization separate from the H-015 Phase 3 attempt 2 work package; document the find in H-016 with proposed fix and seek explicit operator ruling before modifying `discovery.py`. Workaround for H-016 if a Phase-2 fix is not authorized in time: a one-off slug-selection diagnostic that parses the date string out of the event title (titles consistently encode date as YYYY-MM-DD suffix in observed data) bypasses the bug for the probe-retry without changing Phase 2 code.** | **✅ Resolved H-016** — Investigation completed at H-016 by reading `/data/matches/9471/meta.json` directly in the pm-tennis-api Render Shell. Confirmed: gateway response has no `eventDate` key; canonical date source is `startDate` (full ISO timestamp). Fix C applied per **D-028**: (a) `discovery.py` line 328 sources `event_date` from `startDate[:10]` going forward; (b) `slug_selector._passes_date_filter` falls back to `start_date_iso[:10]` for the ~116 historical meta.json files written H-007 through H-016 with empty `event_date`. Both fixes covered by 9 new tests (3 regression in TestParseEvent + 6 fallback in test_stress_test_slug_selector). Subsidiary finding: `_check_duplicate_players` has been silently broken in production since H-007 (always returned False due to empty event_date short-circuit on line 424); restored automatically going forward by the fix. Historical 116+ files retain stale `duplicate_player_flag=False` and cannot be retroactively corrected. |
| **I-017** | **Two pre-existing test failures in `tests/test_discovery.py::TestVerifySportSlug`: `test_network_failure_raises_system_exit` and `test_empty_sports_list_raises_system_exit` both expect `SystemExit` but the production code in `src/capture/discovery.py::verify_sport_slug` raises `RuntimeError`. Surfaced at H-016 during D-028 test runs. Failures exist on the baseline `main` branch at H-016 open (commit 274cd09) — they are not introduced by D-028. Root cause: code-vs-test drift (one of them was changed without updating the other; git history would show which session). Production behavior: `verify_sport_slug` is called at startup; on failure it raises an exception that propagates up. Whether `SystemExit` or `RuntimeError` is raised has no functional difference (both crash the service at startup), but the test-vs-code mismatch means the test suite is not actually validating the intended behavior.** | **4** | **Surface at H-017. Two options: (a) update tests to expect `RuntimeError` (matches current code; smaller change); (b) update code to `raise SystemExit` (matches original test intent; more conventional for fatal startup failures). Either is a small fix with no observation-window implications. Out of scope for D-028.** | **✅ Resolved H-018** per **D-031**. `git blame` at H-018 confirmed the drift direction: commit `bae6ee8e` (peterlitton, 2026-04-18 20:46:03, pre-H-004, pre-single-authoring-channel) deliberately replaced `raise SystemExit(1)` with `raise RuntimeError(...)` at both sites; commit message is explicit: *"fix: replace SystemExit with RuntimeError in verify_sport_slug"*. The drift is not someone being careless — it is a test file that never caught up with an intentional code change made before the single-authoring-channel governance discipline was in force. Option (a) applied: tests renamed to `test_*_raises_runtime_error` and reassertions changed to `pytest.raises(RuntimeError)`; inline comments added citing D-031 and `bae6ee8e`. No code change to `discovery.py`; no change to `main.py`. Verification: `tests/test_discovery.py` 49/49 passed in fresh venv against pinned deps. |
...
*RAID.md updated at H-018 session close. Changes this session:*
- *I-017 marked Resolved per D-031. Blame evidence confirmed intentional pre-H-004 `SystemExit`→`RuntimeError` code change (commit `bae6ee8e`); Option (a) applied — tests updated to match production behavior. Test-file change only; no Phase 2 code touch.*

*Prior update at H-016 session close. Changes that session:*
- *I-016 marked Resolved per D-028 (event_date extraction fix in discovery.py + start_date_iso fallback in slug_selector). Subsidiary finding on _check_duplicate_players documented in the I-016 status cell and in D-028.*
- *I-017 added (sev 4 — TestVerifySportSlug pre-existing test failures). Out of scope for D-028; H-017 disposition.*

*Prior update at H-015 session close:*
- *Issue I-016 added (sev 6 — empty event_date in meta.json)*

*Prior update at H-010 session close. New items:*
