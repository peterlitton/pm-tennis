...
# ---- Session accounting ----
sessions:
  last_handoff_id: H-020
  next_handoff_id: H-021
  sessions_count: 20
...
open_items:
  ...
  resolved_operator_decisions_current_session:
    # Pruned to current-session (H-020) entries only per the H-014-settled
    # stricter-reading convention. H-019 entries are preserved in
    # Handoff_H-019 §2 and STATE v17's prose; this field reflects only
    # the current session's in-session rulings.
    - id: H-020-checkpoint-1-scaffolding-approved
      resolved_session: H-020
      resolved_by_decision: operator ruling at checkpoint 1 pause
      resolution_summary: "Checkpoint 1 scaffolding (module header with [E]/[F]/[G]/[H] citations, exit codes, wire-format constants, SweepConfig + load_sweep_config, CellSpec dataclass + build_default_grid producing 8 cells with M4 control first, placeholder-slug synthesis, build_slug_list_for_subscription) approved with three flag rulings. (a) Placeholder slug format `aec-ph-<hex>-<hex>-2099-12-31` approved — three independent signals of non-real (ph tour token + 8-char abbreviations + far-future date), any one sufficient, combination unambiguous; SHA-256 deterministic per reproducibility requirement; don't revise unless M4 pilot surfaces a reason. (b) Exit codes approved — starting sweep-specific codes at 4 (EXIT_SWEEP_FAILED), preserving probe's 1/2/3 unused in sweeps namespace; 10/11/12 for config/no-candidate/no-anchor classes; EXIT_NO_ANCHOR_SLUG (12) cleanly routes both markets.list() empty and --seed-slug missing. (c) Observation window 30s approved as default — middle of §16.10's 30-60s range; 4 minutes of observation time across 8 cells comfortably inside §16.5's ~30-minute envelope; SWEEP_OBSERVATION_SECONDS env override preserves operator flexibility. Not a DJ-entry threshold — scaffolding decisions are code-turn mechanics, not rule changes."
    - id: H-020-checkpoint-2-and-D-032
      resolved_session: H-020
      resolved_by_decision: operator ruling at checkpoint 2 pause after Reading-B surfacing
      resolution_summary: "Checkpoint 2 surfaced §16.7 clean-(iii) Meaning-vs-Rule column ambiguity during mechanical transcription of the classification state machine. Heartbeat-only + zero-anchor-traffic cell satisfies clean-(iii) under Rule column's literal reading (Reading A), but degraded row's anchor-zero-traffic anomaly makes step 6 textually specified; under Reading A the anomaly becomes operationally unreachable. Claude surfaced three readings (A Rule-column literal, B Meaning-column literal, C veto-pattern) and recommended B. Operator ruled Reading B — §16.5's anchor rationale is for M1-attributable traffic via §14.3 marketSlug echo; heartbeat is connection-level and not subscription-attributable; a cell with heartbeats but zero anchor market_data/trade is meaningfully degraded. Reading A textually names but operationally unreachable anomaly is the inconsistency signal; Reading C over-engineers. D-032 landed at top of DJ — scoped narrowly to clarify §16.7 clean-(iii), not supersede §16 (which remains frozen per §16.11). Fix: `_cell_has_any_traffic` renamed to `_cell_has_anchor_slug_traffic_qualifying`, step 5 default-cell branch uses anchor-specific predicate. Regression test written (heartbeat-only + zero anchor → degraded) becomes the standing regression."
    - id: H-020-checkpoint-3-and-D-033
      resolved_session: H-020
      resolved_by_decision: operator ruling at checkpoint 3 hard pause after SDK exception-surface surfacing
      resolution_summary: "Checkpoint 3 SDK introspection (post-`pip install` of polymarket-us==0.1.2) surfaced 12 exception classes in polymarket_us.__all__ where [E]'s README Error Handling example illustrates 6. Claude surfaced the full hierarchy (PolymarketUSError root; APIStatusError base for 6 4xx/5xx leaves; APIConnectionError base for APITimeoutError; WebSocketError on its own branch), framed this as exit-state-B material (the gap is in what §16.9 step 1 asks for, not how the re-fetch was performed), and proposed frozenset expansions (PermissionDeniedError → rejected; InternalServerError + WebSocketError → transport) per HTTP-semantic partition (4xx client-refusal vs 5xx server-infrastructure + network + WS-layer) + base classes stay in catch-all. Four alternatives considered (stay-with-six, expand-by-isinstance, defer-WebSocketError, InternalServerError-as-rejected); selected named-expansion middle path. D-033 landed above D-032 — scoped narrowly to §16.1 / §16.4 / §16.7 clarification without superseding §16 (which remains frozen per §16.11). Commit: DOCUMENTED_REJECTED_EXCEPTION_TYPES expanded to 5, DOCUMENTED_TRANSPORT_EXCEPTION_TYPES expanded to 5, module header [I] citation added for installed-module __all__ as authoritative public surface. Per-type regression tests (5 rejected + 5 transport + 3 base + frozenset pin = 14 tests) constructed using actual SDK classes with httpx.Response stubs — mirrors D-032's regression pattern. Separate §16.9 standing-convention addendum established via Handoff_H-020 §4: future code-turn sessions perform both README re-fetch AND installed-module introspection, single exit-state rule across both layers."
    - id: H-020-session-close-shape-A
      resolved_session: H-020
      resolved_by_decision: operator ruling at session-close surface
      resolution_summary: "Shape A selected — ship sweeps code and tests, defer live smoke to H-021 per §16.8 items 3+4. Rationale: H-020's cut at session open was sweeps code only; Shape A honors that cut honestly (items 1+2 met: 91 new tests + 113 existing all passing + code reviewable; items 3+4 explicitly deferred). Shape B (defer close, wait for live smoke, unified H-020+H-021 closure) rejected — would erode the one-deliverable-per-session pattern load-bearing since H-013/H-014. Two DJ entries (D-032, D-033) land with this close. STATE bumps v17 → v18. Eleven consecutive clean-discipline sessions (H-010 → H-020). §16.9 standing-convention addendum wording approved with single-exit-state-rule tightening: 'If either layer surfaces a gap against the research record, default to exit state B per H-019's tightening' (per-layer exit-state framing rejected in favor of session-level ruling on the full re-fetch)."
...
scaffolding_files:
  STATE_md:
    status: accepted
    current_version: 17
    committed_to_repo: pending  # v17 produced this session
    committed_session: H-019
    note: "v17 reflects H-019: item 1 complete (POD-H017-D029-mechanism resolution-path check per D-030 — search_mcp_registry returned no GitHub MCP connector, unchanged from H-016/H-017/H-018; POD remains open low urgency); item 2 complete (SDK README re-fetch [E] + docs.polymarket.us/api-reference/websocket/markets re-fetch [F] + PyPI version check [G] per libraries.io — SDK at polymarket-us==0.1.2 is still current, 10 commits on GitHub unchanged from H-013, every probe.py [A]-[D] citation still resolves, no research-record contradictions); item 3 complete (§16 main-sweeps-scope addendum produced as additive to v4, §§1-15 byte-identical, 345-line addition, two rounds of operator review with three amendments landing cleanly in round 2 — M4 control cell 100P/0R added to grid, subscribe-success-ratio classification threshold pinned explicitly, §16.11 explicit `§16 does not amend §§1-15` bullet). Zero DJ entries per operator direction. DJ at 31 entries (unchanged). RAID 13 open / 17 total (unchanged). Ten consecutive clean-discipline sessions (H-010→H-019). H-020 inherits: re-fetch [E] at code-turn time + sweeps code per §16 + surface POD. v16 reflected H-018 close."
  Orientation_md:
    status: accepted
  ...
  Handoff_H019_md:
    ...
    committed_session: H-019
    note: "Produced at H-019 session close per Playbook §2. Bundle includes STATE.md v17, docs/clob_asset_cap_stress_test_research.md (v4 + §16 additive, complete replacement, §§1-15 byte-identical), and this handoff. Zero DJ entries this session per operator direction. All files delivered as complete replacements per D-029 §3 / H-016 always-replace convention. Operator delivery path per D-030 interim flow: drag-and-drop to claude-staging → operator diff-review → merge staging → main."
  data_dictionary_md:
    status: not-started
...
*End of STATE.md — current document version: 17. Last updated: H-019.*
