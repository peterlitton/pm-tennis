...
# ---- Session accounting ----
sessions:
  last_handoff_id: H-019
  next_handoff_id: H-020
  sessions_count: 19
...
open_items:
  ...
  resolved_operator_decisions_current_session:
    # Pruned to current-session (H-019) entries only per the H-014-settled
    # stricter-reading convention. H-018 entries are preserved in
    # Handoff_H-018 §2 and STATE v16's prose; this field reflects only
    # the current session's in-session rulings.
    - id: H-019-session-cut-at-open
      resolved_session: H-019
      resolved_by_decision: operator direction at session open
      resolution_summary: "H-019 scope cut to §16 research-doc addendum only, per Claude's default-offer recommendation matching D-019 research-first-then-code sequencing. Operator approved the narrow cut and explicitly declined the bigger 'addendum + operator review + code turn' shape. One-deliverable-per-session pattern preserved. Main sweeps code defers to H-020. Item 3 (POD-H017-D029-mechanism surface per D-030) included in session scope as brief check, not as separate deliverable. Optional DJ footer cleanup included as standing opportunity but ruled out mid-session per operator direction ('§16 is the deliverable; give it your full attention'). Not a DJ-entry threshold; logged as operational direction."
    - id: H-019-checkpoint-exit-state-A-clean
      resolved_session: H-019
      resolved_by_decision: operator ruling at mid-session checkpoint following SDK README re-fetch
      resolution_summary: "Per operator's tightened step-4 framing of exit states (clean = §16 drafts / contradiction = DJ entry lands, §16 defers). Re-fetch of github.com/Polymarket/polymarket-us-python README at session start established clean baseline: SDK at polymarket-us==0.1.2 still current on PyPI per libraries.io (last release 2026-01-22, two releases total, 0.1.1 and 0.1.2 only); GitHub repo shows 10 commits (unchanged from H-013's '10 commits' recorded in §§12, 15.1); every probe.py [A]-[D] citation still resolves; exception surface identical; Ed25519 signing remains fully internal; multi-subscription composition on one markets_ws demonstrated in README (two subscribe calls of different subscription types on one markets_ws, three on one private_ws); 100-slug-per-subscription cap re-confirmed in docs.polymarket.us. Two empirical gaps surfaced but are not contradictions with research record: README demonstrates multi-type subscription composition, not multi-MARKET_DATA-with-distinct-slug-lists (M1); README demonstrates cross-factory concurrency on one client, not two client.ws.markets() calls on one client (M2). Framed correctly as measurement questions for the harness, not prose observations. Operator ruled exit state A — clean — and authorized §16 drafting. Not a DJ-entry threshold per operator direction ('re-fetch was execution of an already-committed standing instruction, not a decision')."
    - id: H-019-section-16-accept
      resolved_session: H-019
      resolved_by_decision: operator acceptance ruling at H-019 close
      resolution_summary: "§16 main-sweeps-scope addendum accepted after two rounds of review. Round 1 surfaced two design choices for operator input: (1) 1+99 anchor-slug-plus-placeholder composition as default vs pure-100-placeholder; (2) adding `degraded` classification vs keeping probe.py's four-way. Operator ruled: keep 1+99 default AND add dedicated 100P/0R M4 control cell to grid (disambiguates M4 measurement without requiring separate invocation); keep `degraded` classification AND pin the degraded-vs-rejected-vs-accepted threshold explicitly in §16.7 (subscribe-success ratio: ≤0.5 rejected, (0.5, 1.0) exclusive degraded, 1.0 candidate for clean). Plus small §16.11 fix: explicitly name that §16 does not amend §§1-15, rather than having readers infer it from byte-identical preservation. Round 2 verified all three amendments landed cleanly and §§1-15 remained byte-identical (zero diff lines 1-780). Final §16 document is 1127 lines (was 782 at v4 open; 345-line addition). Not a DJ-entry threshold per operator direction — §16 is research-doc production, not a rule change. Governance trail for the re-fetch lives in §16.1's fetch-record header and in the H-019 handoff self-report."
  phase_3_attempt_2_notes:
    ...
    - "H-019 open: full reading completed (Handoff_H-018 all 281 lines, Orientation, Playbook §§1-13 end-to-end, STATE.md v16 YAML + prose, DecisionJournal all 31 entries D-001 through D-031, RAID.md in full, SECRETS_POLICY §§A.1-A.7 + B.1-B.7, research-doc §§7 + 13 + 14 + 15, probe.py header/citations [A]-[D], H-018-Claude informal letter). Session-open self-audit surfaced zero discrepancies — H-018 bundle merged cleanly on main: DJ at 31 entries (D-031 at top), STATE current_version: 16, RAID I-017 Resolved, tests/test_discovery.py lines 566+581 show renamed `test_network_failure_raises_runtime_error` and `test_empty_sports_list_raises_runtime_error` methods on disk. Nine-session clean-discipline streak (H-010→H-018) intact at session open."
    - "H-019 items 1-4 brief-prep (per operator request after first scope proposal): read PM-Tennis_Build_Plan_v4.docx §5.4 ('Pool size is bounded by the soft 150-asset cap (to be stress-tested in Phase 3)') and §11.3 ('soft 150-asset pool cap (2) — stress test deferred to Phase 3') for the I-015 pending-revision context; read probe.py lines 120-764 in full for the sweeps-extension pattern (ProbeOutcome dataclass, six event handlers registered before connect/subscribe, subscription_request_id generation, four-way classification state machine, run_probe CLI dispatch per D-027); read src/stress_test/requirements.txt (polymarket-us==0.1.2, pytest==8.3.4 — pins confirmed against STATE); read runbooks/Runbook_RB-002_Stress_Test_Service.md in full for the two-shell workflow pattern the sweeps invocation mirrors. ~45 min focused reading. Produced detailed preparedness report surfacing the load-bearing question for §16: whether multiple `subscribe()` calls on one `markets_ws` compose additively or replace."
    - "H-019 item 1 (POD-H017-D029-mechanism per D-030): search_mcp_registry(['github','git','repository']) returned GitLab, Contentsquare, Exa, Microsoft Learn, Lucid, Mem, Glean — no GitHub MCP connector. Unchanged from H-016/H-017/H-018 findings. No path-change detected; POD-H017-D029-mechanism remains open, low urgency. No targeted DJ entry per D-030's 'if material' clause — nothing material."
    - "H-019 item 2 (SDK re-fetch per D-019 research-first and R-010 preventive-fabrication): re-fetched `github.com/Polymarket/polymarket-us-python` [E] (fetch URL in §16.1) at session mid-point per operator's tightened exit-states framing. Findings: 10 commits on main (unchanged from H-013's 10 commits recorded in §§12, 15.1); PyPI polymarket-us==0.1.2 is still the latest release per libraries.io [G] (two releases total, 0.1.1 and 0.1.2, both 2026-01-22). Re-fetched docs.polymarket.us/api-reference/websocket/markets [F] — confirmed 100-slug-per-subscription cap language directly: 'You can subscribe to a maximum of 100 markets per subscription. Use multiple subscriptions if you need more.' Cross-checked 12 claims against research record; all consistent. Two empirical gaps identified (M1 multi-same-type subscription composition, M2 multi-client.ws.markets() independence) are measurement questions for the sweeps harness, not contradictions with research record. Operator ruled exit state A (clean) after checkpoint, authorized §16 drafting."
    - "H-019 item 3 (§16 main-sweeps-scope addendum draft): produced 345-line additive section to `docs/clob_asset_cap_stress_test_research.md` per §13/§14/§15 precedent. §§1-15 preserved byte-identical (verified: 0 diff lines against v4 for lines 1-780). §16 organized into 11 subsections covering: fetch record (§16.1 — [E] URL+commit-count+fetch-date, [F], [G] PyPI version metadata); what §16 inherits from prior rulings (§16.2 — D-018 through D-031 enumerated); 100-slug cap re-citation directly from [F] (§16.3); five named measurement questions (§16.4 — M1 multi-same-type subscription composition, M2 multi-client.ws.markets() independence, M3 100-slug cap positive+101-slug negative control, M4 placeholder-slug rejection, M5 concurrent-connection upper bound); harness design (§16.5 — src/stress_test/sweeps.py entry point, CLI surface, 1+99 default composition + dedicated 100P/0R M4 control cell); outcome record shapes (§16.6 — SweepCellOutcome/ConnectionObservation/SubscribeObservation/SweepRunOutcome); classification state machine (§16.7 — five-way with subscribe-success-ratio threshold pinned explicitly); acceptance bar (§16.8 — unit tests + operator code review + live smoke run including M4 control cell); session-close discipline for H-020 code turn (§16.9 — re-fetch [E] again at code-turn time per R-010); what §16 does not decide (§16.10); what §16 does not change (§16.11 — explicitly names that §16 does not amend §§1-15)."
    - "H-019 item 4 (operator review round 1): operator accepted draft shape; ruled on two design choices surfaced as open: (1) keep 1+99 anchor-slug-plus-placeholder as default AND add dedicated 100P/0R M4 control cell to grid as M4 unconfounded measurement; (2) keep five-way classification with `degraded` AND pin degraded-vs-rejected-vs-accepted threshold explicitly (subscribe-success ratio: ≤0.5 rejected, (0.5, 1.0) exclusive degraded, 1.0 candidate for clean). Plus small §16.11 explicit-naming fix (state byte-identical preservation, not infer it)."
    - "H-019 item 5 (§16 amendments): three changes applied. (a) §16.5: dedicated M4 control cell (100P/0R single-subscription single-connection) added alongside main grid; flagged via cell_id='m4-control-100p-0r' and SubscribeObservation.real_slug=''. (b) §16.7: subscribe-success-ratio threshold pinned as 7-step classification precedence list; special cases for single-subscribe cells (no `degraded` possible) and M4 control cell (relaxed `clean` — zero anchor traffic expected). (c) §16.8: M4 control cell added to acceptance-bar live-smoke checklist as its own bullet. (d) §16.11 first bullet rewritten to explicitly name '§16 does not amend §§1-15' rather than just 'no claim revised'. Re-verified §§1-15 still byte-identical (0 diff lines against v4). Re-verified section header integrity (16 sections at expected line positions)."
    - "H-019 close: zero DJ entries added (per operator direction — re-fetch was execution of already-committed standing instruction, not a decision; §16 is research-doc production, not a rule change). STATE bumped v16 → v17. DJ counter unchanged at 31. RAID unchanged (13 open / 17 total). Tripwires: none fired. OOP: zero. Ten consecutive clean-discipline sessions (H-010 → H-019). Mid-session `present_files` lesson from H-017 still held — only one present_files call (at the operator-requested review step), not mid-session. H-018-Claude's 'fourth consecutive handoff, will be the first to actually execute it' README re-fetch standing instruction was finally exercised; instruction framing in §16.9 now shifts to 'always re-fetch [E] at code-turn time' for H-020+. H-020 picks up: (a) re-fetch [E] per §16.9 step 1; (b) sweeps code `src/stress_test/sweeps.py` + `tests/test_stress_test_sweeps.py` implementing §16 as operator-reviewed; (c) live smoke run likely H-021 per one-deliverable-per-session pattern; (d) surface POD-H017-D029-mechanism per D-030 Resolution path."
